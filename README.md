...
在 UniFi Web UI 中为每个运营商创建一条 Traffic Route，记住：
- **description**（规则名称）
...
| 脚本 | 规则名称 | ipset | 数据源 |
|------|----------|-------|--------|
| `update_first_mobilev4.sh` | `First_mobilev4` | `UBIOS4trafficroute_ip_3` | 中国移动 |
| `update_first_telecomv4.sh` | `First_telecomv4` | `UBIOS4trafficroute_ip_4` | 中国电信 |

如果你的规则名称或 short_id 不同，修改对应脚本中的变量：
...
```
scripts/
  update_first_mobilev4.sh    # 移动更新脚本：下载 rsc -> 提取 IP -> 更新 MongoDB + ipset
  update_first_telecomv4.sh   # 电信更新脚本：同上
...
```
/data/custom/
  update_first_mobilev4.sh    # 移动更新脚本
  update_first_mobilev4.log   # 移动运行日志（自动保留最近 100 条）
  update_first_telecomv4.sh   # 电信更新脚本
  update_first_telecomv4.log  # 电信运行日志
...
默认每天凌晨执行更新，错开 10 分钟避免并发：

| 时间 | 脚本 |
|------|------|
| 04:30 | `update_first_mobilev4.sh` (移动) |
| 04:40 | `update_first_telecomv4.sh` (电信) |
